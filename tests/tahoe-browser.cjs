...
(async()=>{
 ...
 const state=await page.evaluate(()=>{const e=window.__ROOM_APP__.environment;return {spatial:e.surface.audit(),seasons:Object.keys(e.state.seasons),animals:e.animals.animals,people:e.village.agents.length};});assert.equal(state.spatial.invalid.length,0);assert.equal(state.seasons.length,4);assert.equal(state.animals.length,0);assert.equal(state.people,3);assert.deepEqual(errors,[]);console.log('PASS room interactions, season APIs and spatial contracts; screenshots require manual visual acceptance');await browser.close();
