...
(async()=>{
 ...
 for(const angle of [0,90,180,270]){
  ...
  assert(result.distance>=10);await page.waitForTimeout(400);await page.screenshot({path:`/tmp/tahoe-orbit-${angle}.png`});
 }
 ...
 for(const mobile of [false,true]){
  ...
  for(const type of ['monitor','notebook','turntable','map','photoWall','books']){
   ...
   const dimensions=await page.locator('#experience-content').boundingBox();assert(dimensions.width<=(mobile?390:900));
 ...
 const state=await page.evaluate(()=>{const e=window.__ROOM_APP__.environment;e.setMode('day');e.update(.1,5);const flying=e.birds.map(b=>b.state);e.setMode('night');for(let i=0;i<60;i++)e.update(.1,30);return {flying,night:e.birds.map(b=>b.state),seasons:Object.keys(e.state.seasons),shader:e.water.material.fragmentShader.includes('cameraPosition-world')};});
 assert(state.flying.includes('flying'));assert(state.night.every(s=>s==='resting'));assert.equal(state.seasons.length,4);assert(state.shader);assert.deepEqual(errors,[]);console.log('PASS day/night/winter, full 360 orbit, drag safety, all 6 panels on desktop/mobile, seasonal/bird state and shader console');await browser.close();
