...
window.AnimalSystem=(()=>{
 ...
 function create(scene,surface,definitions=AnimalPopulation){
  ...
  const api={root,animals,pickables,status,ready:null,get quality(){return quality;},update(dt,time,world){if(disposed)return;deferred+=dt;const budget=quality==='low'?1/30:0;if(dt&&deferred<budget)return;for(const a of animals)a.update(dt?deferred:0,world,time);if(dt)deferred=0;syncPickables();},setQuality(v){quality=v;animals.forEach(a=>a.setQuality(v));syncPickables();},describe(a){return a.label+' · '+(labels[a.state]||a.state)+'\n'+a.story;},dispose(){if(disposed)return;disposed=true;animals.forEach(a=>a.dispose());animals.length=0;pickables.length=0;scene.remove(root);if(active.get(scene)===api)active.delete(scene);}};
  ...
  api.ready=Promise.all(definitions.map(async(d,i)=>{
   ...
   try{asset=await AnimalLoader.acquire(d);if(disposed){asset.release();return;}const actor=AnimalActor.create(asset,d,surface);actor.setQuality(quality);root.add(actor.group);animals.push(actor);status[i].status='ready';syncPickables();}
