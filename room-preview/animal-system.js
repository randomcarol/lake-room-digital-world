...
window.AnimalSystem=(()=>{
 ...
 function create(scene,surface,definitions=AnimalManifest){
  ...
  const api={root,animals,pickables,status,ready:null,get quality(){return quality;},update(dt,time,world){if(disposed)return;deferred+=dt;const budget=quality==='low'?1/30:0;if(deferred<budget)return;for(const a of animals)a.update(deferred,world);deferred=0;syncPickables();},setQuality(v){quality=v;animals.forEach(a=>a.setQuality(v));syncPickables();},describe(a){return a.label+' · '+(labels[a.state]||a.state)+'\n'+a.story;},dispose(){if(disposed)return;disposed=true;animals.forEach(a=>a.dispose());animals.length=0;pickables.length=0;scene.remove(root);if(active.get(scene)===api)active.delete(scene);}};
